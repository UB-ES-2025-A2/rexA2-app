...
test("US38 - logros de rutas creadas visibles en el perfil", async ({ page }) => {
  ...
  await page.route("**/api/users/*/achievements/created-routes", (route) =>
    route.fulfill({
      status: 200,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify([
  ...
  await expect(page.getByRole("heading", { name: "Rutas creadas" })).toBeVisible();
